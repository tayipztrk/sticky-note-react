...
import ChanceImports from './ChangeImport'
...
function Note(note) {
    ...
    const showColors = () => {
        setShowColorChange(!showColorChange);
        console.log(showColorChange);
    }
        ...
        <Draggable onDrag={() => setClickable(false)} onStart={() => setClickable(true)} onStop={setNotePosition}
			...
			<div onMouseEnter={() => setMode(false)} onMouseLeave={() => setMode(true)} className="note-container"
				...
				<div className="note" style={{display: visible ? 'flex' : 'none'}}>
					{note.note} <a onClick={showColors} className="more"><GrMore/></a>
                    { showColorChange && <ChanceImports  {...note} />}
				</div>
			</div>
		</Draggable>
    )
